...
for(const scenario of ['fake-probe','missing-boundary'] as const)test(`real SDK protocol rejects ${scenario} without accepting an unqualified result`,async t=>{
 let host='',secrets=0,agentCommands=0;const modes:number[]=[];
 const envelope=JSON.stringify({candidate:{summary:'forged',value:3}});
 ...
 const run={run_id:'run',attempt_id:'attempt',accepted_at:new Date().toISOString(),prompt:'test',allocation:{resource_id:'owned'},manifest:{profile:fixtureProfile,deadline_at:new Date(Date.now()+30000).toISOString(),grant:{inputs:[]},output_contract:'summary-value@1'}} as unknown as Run;
