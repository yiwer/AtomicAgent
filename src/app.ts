...
interface AppOptions { database: string; profile: Profile; identities: Identity[]; sandbox: SandboxPort }
...
async function body(request: IncomingMessage): Promise<unknown> {
  ...
  for await (const chunk of request) {
    bytes += chunk.length;
    if (bytes > 16_384) throw new ApiError(413, 'request_too_large');
    chunks.push(chunk);
  }
  ...
  const server = createServer(async (request, response) => {
    ...
    try {
      ...
      if (path === '/internal/health' && method === 'GET') {
        ...
        send(response, 200, { observed_at: now(), source: 'platform:durable-runs', coverage: 'ticket01',
      ...
      if (path === '/v1/runs' && method === 'POST') {
        ...
        if (Object.keys(input).some(k => !['prompt', 'profile', 'output_contract'].includes(k)) ||
        ...
        if (input.output_contract !== 'summary-value@1') throw new ApiError(400, 'output_contract_unavailable');
        const acceptedAt = now();
        const run: Run = {
          run_id: randomUUID(), owner: identity.actor, workspace: identity.workspace, prompt: input.prompt,
          request_digest: digest(JSON.stringify([input.prompt, input.profile, input.output_contract])).toString('hex'),
          manifest: { profile: structuredClone(options.profile), output_contract: 'summary-value@1', schema: outputSchema,
            grant: { tools: [], mcp: [], inputs: [], external_access: 'model-only' },
      ...
      if (match && method === 'GET') {
        ...
        if (match[2]) {
          ...
          send(response, 200, { run_id: run.run_id, result: run.result, artifacts: [], validation: run.validation });
