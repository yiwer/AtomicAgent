...
import { researchServer, requestedMcp, researchSchema, validateResearch, researchMarkdown, type McpBinding } from './research.js';
...
export async function runResearch(request: ClaudeRequest, root: string, runQuery: QueryPort) {
 ...
 try {
  ...
  const service = researchServer(binding, controller.signal, async (receipt, event) => {
   try {
    ...
    if (event === 'denied') { evidence.authorized = false; failed = true; }
  ...
  const messages = runQuery({ prompt: `Task request:\n${request.prompt}`, options: {
   ...
   canUseTool: async (name, input) => permitted(name, input) ? { behavior: 'allow', updatedInput: input } : { behavior: 'deny', message: 'authorization_required' },
   hooks: { PreToolUse: [{ hooks: [async input => ({ hookSpecificOutput: { hookEventName: 'PreToolUse', permissionDecision: input.hook_event_name === 'PreToolUse' && permitted(input.tool_name, input.tool_input as Record<string, unknown>) ? 'allow' : 'deny', permissionDecisionReason: 'registered-readonly-source-only' } })] }] },
