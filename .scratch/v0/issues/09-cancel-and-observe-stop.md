# 09 — 取消任务并核验实际停止

**What to build:** 调用方或获授权维护者从 API／A 请求取消，持续看到取消裁定、实际停止及独立清理结果。

**Blocked by:** 01 — 提交提示词并在 A 中取回已校验 JSON

**Status:** ready-for-agent

**范围来源：** PRD + A；2026-09-14 用户接受 v0＋A 拆分后发布。

**验收对应：** AC-07。

**管理台验收对应：** A-AC-01。

## Acceptance criteria

- [ ] 排队中取消不产生 Attempt 或 sandbox；准备中取消回收已产生资源；运行中先记取消意图，30 秒宽限后执行层强制停止。
- [ ] 取消与结果提交共用原子终态裁定，只有一个权威终态；迟到取消或旧事件不能覆盖已提交成功。
- [ ] 重复取消安全可查；实际停止未知时页面保留未知／待核验，不假装资源已删除，所有相关清理责任持续存在。

## 验证与证据

排队／准备／执行中取消、完成竞争及迟到事件；真实核对进程停止和资源状态。

每票交付可运行命令、最短演示步骤和输入／动作／可观察结果／证据出处。分别记录确定性实验、真实集成及本票适用的浏览器／部署／内容验收；所有验收当前未执行。

## 贯穿本票的边界

- 随本票行为交付权限、关键事实持久化、失败与重复请求处理、所属审计事件及带新鲜度的健康／异常来源。观测出口使用有界字段允许清单。
- 一个 Run 至多一次实际 Agent 执行和一个权威终态；Result、Artifact 可用性和实际回收独立。未知结果先对账，业务重做须新 Run，阶段重试不重置累计限制。
- 页面与 API 查询同一事实；管理操作由服务端授权。提供刷新、越权和错误恢复证据，不以模拟控件或替身结果代替真实路径。

## Comments

- 2026-09-14：根据用户“接受”发布；尚未开始产品实现或验收。
- 2026-09-14：按 `$implement` 完成 API/A、原子取消终态、30 秒宽限、真实停止独立核验、未知退避/重启和准备/复制晚完成责任；[实施说明](../../../docs/implementation/ticket09.md)及[分层证据](../evidence/ticket09/verification.md)。当前确定性回归114/114、取消专属9/9及真实Windows子进程30.085秒停止通过，Chromium聚焦通过；全浏览器回归和双轴审查正在记录。获准模型/Linux OpenSandbox物理验收、人工综合验收仍未完成，AC不勾选。
- 2026-09-14：实施及双轴审查完成。checkpoint `7e21860ae1fa136860fdf62b4b9ec73336af91ef`，修复 `56e940d06c3705cd0f65bb876f4e273fcbe06401`；最终类型／构建、全套119/119、Chromium17/17通过；取消专属10/10、provider协议6/6；Standards／Spec复核各0剩余问题。保留真实Windows受控进程证据；获准模型／Linux sandbox／人工综合验收尚缺，实施完成不关闭AC。
